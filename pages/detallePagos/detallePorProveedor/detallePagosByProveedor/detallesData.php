...
if (!empty($_POST)) {
    ...
    if ($fechaIni === "-1") { //ver todas  las compras
        $query = "SELECT * FROM %s WHERE Eliminado IS NULL AND IdProveedor = ". $_POST['idProveedor'];
        $dataTable->construyeConsulta(sprintf($query, $dataTable->nombreTabla));
    }
    else {
        $query = "SELECT * FROM %s WHERE Eliminado IS NULL AND (Creado BETWEEN '%s' AND '%s') AND IdProveedor = ". $_POST['idProveedor'];

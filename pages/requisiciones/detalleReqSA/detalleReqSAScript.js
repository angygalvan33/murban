function inicializaDetalleReqSATable(permisoCancelar) {
    $('#detalleReqSATable').DataTable( {
        'processing': true,
        'serverSide': true,
        "bDestroy": true,
        'ajax': {
            ...
            error: function() { //error handling
                $(".detalleReqSATable-error").html("");
                $("#detalleReqSATable").append('<tbody class="detalleReqSATable-error"><tr><th colspan="3">No hay datos para mostrar</th></tr></tbody>');
                $("#detalleReqSATable_processing").css("display", "none");
            },
            data: {
    ...
    $.post("./pages/requisiciones/datos.php", datos, function(result) {
        switch(result["error"]) {
            case 0:
                ...
                $('#detalleReqSATable').DataTable().ajax.reload();
            break;
            case 1:

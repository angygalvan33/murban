    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <input type="hidden" name="idOC" id="idOC">
                ...
                <div style="width: 70% !important; margin: auto">
                    <form id="pagarForm">
                        ...
                        <div class="form-group" id="divMetodoPago">
                            ...
                            <label id="errorCantidad" style="display:none; color:red">La cantidad a pagar es mayor a la deuda</label>
                        </div>
                        <div class="form-group" id="divMetodoPago">
                            <label>Cantidad Factura(MXN):</label>
                            <input type="text" id="cantidadFact" name="cantidadFact" class="form-control" required>
                            <label id="errorCantidad" style="display:none; color:red">La cantidad a pagar es mayor a la deuda</label>
                        </div>
                        <div class="form-group" id="divMetodoPago">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" onclick="validarFormulario($('#idOC').val(), $('#metodoPago').val(), $('input[type=radio][name=tpago]').val(), $('#cantidad').val(), $('#cantidadFact').val(), $('#concepto').val(), $('#pDeuda').text().replace(/\,/g, ''), $('#fechaPagoH').val())">Pagar</button>
            </div>
        </div>
        <!-- /.modal-content -->
    </div>
...
<script type="text/javascript">
    ...
    function validarFormulario(idOC, metodoPago, tpago, cantidad, cantidadFact, concepto, deuda, fechaPago) {
        if ($("#pagarForm").valid()) {
            if (esCantidadMenorOIgualADeuda($("#cantidad").val().replace(/\,/g, ''), $("#pDeuda").text().replace(/\,/g, ''))) {
                $("#errorCantidad").css("display", "none");
                pagarOC(idOC, metodoPago, tpago, cantidad, cantidadFact, concepto, deuda, fechaPago);

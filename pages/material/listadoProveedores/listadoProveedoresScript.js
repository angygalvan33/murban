function loadListadoProveedoresDataTable(editarPrecio) {
    $('#listadoProveedoresTable').DataTable( {
        'processing': true,
        'serverSide': true,
        'ajax': {
            url: "pages/material/listadoProveedores/listadoProveedoresData.php", //json datasource
            type: "post", //method, by default get
            data: {
                "IdMaterial": $(".detalles").attr("id")
            },
            error: function() { //error handling
                $(".listadoProveedoresTable-error").html("");
                $("#listadoProveedoresTable").append('<tbody class="listadoProveedoresTable-error"><tr><th colspan="3">No hay datos para mostrar</th></tr></tbody>');
                $("#listadoProveedoresTable_processing").css("display", "none");
            }
        },
        'columns': [
            { 'data': "Proveedor", orderable: true, width: "30%" },
            { 'data': "Precio", orderable: true, width: "15%",
                mRender: function (data, type, row) {
                    return "$"+ formatNumber(row.Precio);
                }
            },
            { 'data': "Iva", orderable: true, width: "5%",
                mRender: function (data, type, row) {
                if (row.Iva == 1)
            ...
            { 'data': "Creado", orderable: true, width: "20%" },
            { orderable: false,
                mRender: function (data, type, row) {
                    if (editarPrecio === true) {
                        var buttons = "<button type='button' id='editarPrecioMat' style='margin-right:5px' class='btn btn-success btn-sm'><i class='fa fa-edit'></i>&nbsp;Editar precio</button>";
                        return buttons;
                    }
                    else
                        return "";
...
function autocompleteProveedores() {
    $('.nproveedor').select2( {
        ajax: {

...
function autoCompleteProveedoresOCReqxkilo() {
    $('#proveedorOCReqxkilo').select2( {
...
function inicializaVistaPreviaOCReqxkilo() {
...
function inicializaMaterialesByProveedorOCReqxkilo() {
    $('#matsOCReqxkilo').DataTable( {
        'processing': true,
        'serverSide': true,
        'ajax': {
            url: "pages/compras/requisicionesxkilo/detalleCompraDataxkilo.php", //json datasource
            ...
            error: function() { //error handling
                $(".matsOCReqxkilo-error").html("");
                $("#matsOCReqxkilo").append('<tbody class="matsOCReq-error"><tr><th colspan="3">No hay datos para mostrar</th></tr></tbody>');
                $("#matsOCReq_processingxkilo").css("display", "none");
            }
        },
        'columns': [
...
function eliminarOCReqxkilo(idReqAtendida) {
    var datos = {};
...
function agregarOCReqxkilo(nuevo) {
    var existe = false;
    var actualRow = null;
...
function guardarOrdenDeCompraOCReqxkilo(tipo, permisoAutorizar, descripcionCompra, notasProveedor, idMetodoPago, idUsuario) {
    var idReqsDetalle = [];
    var idReqsAtendida = [];
    ...
    $('#matsOCReqxkilo').DataTable().rows().every( function () {
        var d = this.data();
        var nuevo = {};

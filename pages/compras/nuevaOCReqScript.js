...
function inicializaVistaPreviaOCReq() {
    $('#ocCompleteTablaOCReq').DataTable( {
        'lengthMenu': [ [25, 30, 35, -1], [25, 30, 35, "Todos"] ],
        'data': [],
        'columns': [
...
function inicializaMaterialesByProveedorOCReq() {
    $('#matsOCReq').DataTable( {
        "lengthMenu": [ [30, 35, 40, -1], [30, 35, 40, "Todos"] ],
        'processing': true,
        'serverSide': true,
        'ajax': {
            url: "./pages/compras/requispreoc/detalleCompraData.php", //json datasource
        ...
        'columns': [
            ...
            { 'data': "UsuarioSolicita", orderable: true, width: "10%" },
            { 'data': "FechaProv", orderable: true, width: "15%" },
            { width: "10%", orderable: false,
                mRender: function (data, type, row) {
                    if (row.IdProyecto > -1)
                         return "<button type='button' id='regresar_OCReq' style='margin-right:5px' class='btn btn-danger btn-sm'>Eliminar</button>";
                    else
                        return "";
                }
            }
        ...
        'footerCallback': function (row, data, start, end, display) {
            ...
            $.each(data, function(key, value) {
                ...
                nuevoOCReq["IdProyecto"] = value.Proyecto;
                agregarOCReq(nuevoOCReq);
            });
            // Update footer
            $( api.column(3).footer() ).html(
                subtotalOCReq.toFixed(3)
            );
            ivaOCReq = subtotalOCReq * 0.16;
            totalOCReq = subtotalOCReq + ivaOCReq;
            $("#subtotalOCReq").text("$"+ formatNumber(subtotalOCReq.toFixed(3)));
            $("#ivaOCReq").text("$"+ formatNumber(ivaOCReq.toFixed(3)));
            $("#totalOCReq").text("$"+ formatNumber(totalOCReq.toFixed(3)));
        }
    });
}
...
function guardarOrdenDeCompraOCReq(tipo, permisoAutorizar, descripcionCompra, notasProveedor, idMetodoPago, idUsuario) {
    ...
    if (datosDetalleCompra.length > 0) {
        ...
        console.log(dataProveedor[0].id);
        switch (tipoPago) {
            case "0": //por pagar

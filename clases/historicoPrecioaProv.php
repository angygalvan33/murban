...
class HistoricoPrecioaProv {
    ...
    public function guardarPrecio($accion)
    {
        ...
        try 
        {
            $conexion->obtenerNuevoIdTabla('HistoricoPrecioaProv');
            $nueviId = $conexion->result['result'];

            if( $conexion->abrirBD()!=NULL)
            {
                $query = "CALL InsertaPrecioaProv('$accion', ".$nueviId.", ".$this->idParte.", ".$comodin->idUsuarioSession().", ".$this->precio.", ".$this->iva.", '$this->moneda', ".$this->idTipo.");";
    ...
    function getPrecioProv($idMaterial, $idTipo){
        $conexion = new Conexion();
        $precio = 0;
        $band_query_exito = 1;
        ...
        try
        {
            if( $conexion->abrirBD() != NULL )
            {
                $query = "SELECT Precio FROM HistoricoPrecioaProv WHERE IdParte = ". $idMaterial ." AND IdTipo=". $idTipo ." ORDER BY Precio DESC LIMIT 1;";

...
class Departamento {
    ...
    public function getDepartamentos()
    {
        ...
        if( $conexion->abrirBD()!=NULL)
        {
            $query = "SELECT IdDepartamento, Nombre FROM Linea WHERE Eliminado IS NULL";
//          echo $query;
    ...
    public function getDepartamentosFilter($busqueda)
    {
        $conexion = new Conexion();
        ...
        if( $conexion->abrirBD()!=NULL)
        {
            if($busqueda != null)
            {
                $query = "SELECT * FROM Departamento WHERE Eliminado IS NULL AND Nombre like '%".$busqueda."%' limit 5;";
            }
            else
            {
                $query = "SELECT * FROM Departamento WHERE Eliminado IS NULL limit 5;";
            }
            ...
            {    
                $data[] = array("id"=>$row['IdDepartamento'], "text"=>$row['Nombre']);
            }
